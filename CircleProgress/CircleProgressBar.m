...
// 起始角度默认为顶部中点
#define DEGREES_TO_RADIANS(angle) (((angle) / 180.0 * (M_PI))-M_PI/2)

// Hint View Defaults
#define DefaultHintBackgroundColor [UIColor colorWithWhite:0 alpha:0.7]

const CGFloat ProgressDimensionalityCoefficient = 6.2f;   // 维度系数
const CGFloat DefaultProgressRadius = 100.0f;             // 跟随路径半径
const CGFloat DefaultHintSpacing = 5.0f;                  // 进度值与外环间距
const CGFloat DefaultHintTempValue = 50.0f;               // 临界值
const StringGenerationBlock DefaultHintTextGenerationBlock = ^NSString *(CGFloat progress) {
    return [NSString stringWithFormat:@"%.0f%%", progress * 100];   // 进度字符串
};

// Animation Constants
const CGFloat AnimationChangeTimeDuration = 0.2f;    // 动画持续时间
const CGFloat AnimationChangeTimeStep = 0.01f;       // 动画进度频率
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat radius = MIN(innerCenter.x, innerCenter.y) - DefaultHintTempValue;
...
- (void)drawSimpleHintTextAtCenter:(CGPoint)center {
    ...
    CGFloat coordinateX;
    CGFloat coordinateY;
    CGFloat centerX = center.x-20;
    CGFloat centerY = center.y-10;
    NSLog(@"progress.value=%f", _progress*ProgressDimensionalityCoefficient);
    
    // 圆上任一点坐标
    coordinateX = centerX + DefaultProgressRadius*sin(_progress*ProgressDimensionalityCoefficient);
    coordinateY = centerY - DefaultProgressRadius*cos(_progress*ProgressDimensionalityCoefficient);
    
    [progressString drawAtPoint:CGPointMake(coordinateX, coordinateY) withAttributes:@{NSFontAttributeName: self.hintTextFontForDrawing, NSForegroundColorAttributeName: self.hintTextColorForDrawing}];
}
